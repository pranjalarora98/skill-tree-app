...
describe('searchUtil', () => {
  ...
  describe('applySearchHighlighting', () => {
    test('returns cleaned classes and default edge styles when searchTerm is empty', () => {
      const nodes = [
        {
          id: '1',
          data: { name: 'One' },
          className: `${SKILL_CONSTANTS.NODE_CLASS_DIRECT_MATCH} ${SKILL_CONSTANTS.NODE_CLASS_PATH_HIGHLIGHT}`,
        },
      ...
      const { highlightedNodes, highlightedEdges } = applySearchHighlighting(
        nodes,
        edges,
        ''
      );
      ...
      expect(
        highlightedNodes.find((n) => n.id === '1').className
      ).not.toContain(SKILL_CONSTANTS.NODE_CLASS_DIRECT_MATCH);
      expect(
        highlightedNodes.find((n) => n.id === '1').className
      ).not.toContain(SKILL_CONSTANTS.NODE_CLASS_PATH_HIGHLIGHT);
    ...
    test('marks direct matches and path highlights and updates edge styles', () => {
      ...
      expect(reactNode.className).toContain(
        SKILL_CONSTANTS.NODE_CLASS_DIRECT_MATCH
      );

      expect(jsNode.className).toContain(
        SKILL_CONSTANTS.NODE_CLASS_PATH_HIGHLIGHT
      );
      expect(otherNode.className).toContain(
        SKILL_CONSTANTS.NODE_CLASS_PATH_HIGHLIGHT
      );
      ...
      const otherReactEdge = highlightedEdges.find(
        (e) => e.id === 'e-other-react'
      );
    ...
    test('does not mark unrelated nodes or edges when no path exists', () => {
      ...
      const edges = [{ id: 'e-ab', source: 'a', target: 'b', style: {} }];
      ...
      expect(highlightedNodes.find((n) => n.id === 'c').className).toContain(
        SKILL_CONSTANTS.NODE_CLASS_DIRECT_MATCH
      );
      expect(
        highlightedNodes.find((n) => n.id === 'a').className
      ).not.toContain(SKILL_CONSTANTS.NODE_CLASS_PATH_HIGHLIGHT);
      expect(highlightedEdges[0].style.stroke).toBe('#00bcd4');
      expect(highlightedEdges[0].style.strokeWidth).toBe(2);
    });
